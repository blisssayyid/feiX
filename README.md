...
##### 1. 首先从获取好友开始，首先清空用户列表刷新

假如在不添加IP段的情况下，飞秋会**多次**使用UDP协议发送如下两条命令到本IP的最后一个IP地址和 255.255.255.255

例： 本机IP地址为 10.211.55.3 ,那么会先发送命令一分别到 10.211.55.255 和 255.255.255.255，紧接着发送命令二到 255.255.255.255, 然后发送**两遍**命令二到 10.211.55.255

假如配置了网段好友IP，那么除了上面的两个IP会发送之外，还会每个IP都发送一次，遍历发送！

例： 本机IP地址为 10.211.55.3 ,配置了好友IP段为 10.11.204.1 ~ 10.11.204.254, 那么会发送命令一分别到 10.211.55.255 10.11.204.1, 10.11.204.2, ..., 10.11.204.254, 255.255.255.255; 然后给 255.255.255.255 发送一条命令二，接着发送**两遍**命令二分别到 10.211.55.255 10.11.204.1, 10.11.204.2, ..., 10.11.204.254

```
命令一： 1_lbt6_3#128#001C429D7F77#0#0#0#4001#9:1513616260:sayyid:CDC0:0:

命令二： 1_lbt6_3#128#001C429D7F77#0#0#0#4001#9:1513616262:sayyid:CDC0:6291457:
```
那么上面这个命令什么意思呢？

经过资料查证之后，`1_lbt6` 是飞秋版本号, `128` 表示等级（就和QQ的等级类似，小太阳或者小月亮）, `001C429D7F77` 表示MAC地址, `#0#0#0#4001#9` 暂时不知道是啥意思, `1513616262` 包序号，这里建议使用时间戳, 因为不可重复, 接下来 `sayyid` 是用户名, `CDC0` 是主机名, `0`暂时不知道什么意思 `6291457`是命令字，表示上线
